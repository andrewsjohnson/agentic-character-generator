...
describe('formatAbilityBonuses', () => {
  // Note: The formatAbilityBonuses function is not exported, so we test it indirectly
  // by checking that species cards don't show a Bonuses line when there are no bonuses.
  // This is because the current races.json has no ability bonuses (2024 SRD rules).

  it('does not display bonuses line when no bonuses exist', () => {
    const mockCharacter = {};
    const mockUpdate = vi.fn();
